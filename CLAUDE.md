...
**Google Sign-In.** `POST /auth/google` verifies a client-obtained Google Identity Services ID token (`security/auth.py`'s `verify_google_id_token`, via `google-auth` — signature-checked against Google's public keys, `aud` must match `GOOGLE_CLIENT_ID`; no client secret needed, so `GOOGLE_CLIENT_SECRET` sits in `backend/.env` unused) and issues the same access/refresh pair as password login. First sign-in for an email creates a `User` with the `password_hash="!"` sentinel (already used by this codebase's own test fixtures for "no real password"); a *second* Google login for that email reuses the row only if it still carries that sentinel — an email that already has a real password from `/auth/register` gets `409`, not a silent login, since registration has no email-verification step and naively merging identities by email would otherwise let someone pre-register a victim's email and hijack their later Google sign-in. Frontend button lives in `Login.tsx`, rendered via the GSI `<script>` tag in `index.html` (no npm package) when `VITE_GOOGLE_CLIENT_ID` is set — a Vite build-time env var, so Docker needs it as a `build.args` entry (`docker-compose.yml`), not a runtime one.
...
**Account deletion.** `DELETE /auth/me` (Settings page, "danger zone") deletes a user and everything tied to them — architecture §6.8, first concrete piece of privacy compliance. Password-confirmed unless the account is Google-only (`password_hash="!"`, which has nothing to confirm). Plaid revocation (`BankingProvider.remove_item`) runs best-effort per `Item` before the local delete — a failed revoke never blocks a user from deleting their own data. No DB-level `ondelete` cascades exist on any FK, so the route deletes in FK-safe order itself (`DetectedSubscription`/`Transaction` → `Account` → `Item`/`RescanJob` → `User`).

**Consent logging.** `GET`/`POST /consent` (`api/routes/consent.py`) track privacy-notice agreement as an append-only `ConsentRecord` log (not a mutable flag) versioned by `CURRENT_VERSION` — bumping it invalidates older grants, forcing re-consent if the disclosure changes materially. Gated in `ConnectBank.tsx` before a *fresh* bank link only (an inline panel, not a modal) — reconnects skip it since consent was already given at first link. **The disclosure copy is placeholder text**, not lawyer-reviewed; data-residency review is the one remaining open piece of privacy compliance and isn't code — it depends on where this gets deployed (`docs/BACKLOG.md`).
