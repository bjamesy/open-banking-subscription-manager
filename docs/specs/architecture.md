...
| 6.8 | Data deletion is **implemented** (`DELETE /auth/me` — Plaid revocation best-effort, then a full local cascade delete in FK-safe order; no `ondelete` cascades at the DB level, done in application code). | Consent logging (no disclosure yet that transaction data is sent to Anthropic) and data-residency review (Postgres location, Anthropic's US-based API) are still open — the latter requires an actual privacy-law review, not just code, and depends on where this gets deployed. |
