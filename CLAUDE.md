...
Also working: refresh tokens (typed access/refresh JWTs, `/auth/refresh`; stateless — no revocation list yet) and `/accounts` and `/transactions` read routes (auth-scoped).
