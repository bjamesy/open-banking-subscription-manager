...
- **Set the webhook URL on link-token creation**: `PlaidProvider.create_link_token`
  doesn't pass `webhook=...`, so Plaid never delivers webhooks — sync currently
  relies on the daily poll. Needs a public HTTPS URL (deploy-dependent) passed
  through config, e.g. `PLAID_WEBHOOK_URL` → `LinkTokenCreateRequest(webhook=...)`.
...
that Item (logged, skipped by poll). Needed: catch Plaid errors in
`ingestion/sync.py`, set `Item.status="error"` + `error` code, surface in the
Accounts UI, and implement Link **update mode** (link token with `access_token`)
...
Root logger is unconfigured — app `logger.info(...)` (scheduler, webhook sync)
is invisible under uvicorn. Configure structured logging (level from env) at
app startup; ensure tokens/PII never logged.

### 6. Cloud deployment
No deploy target chosen. Deciding forces §6.3: the in-process APScheduler
assumes **single instance** — multi-instance requires moving the poll to an
external scheduler/queue (Celery + Redis or a cron hitting an admin endpoint).
Also needed: TLS, secrets management (not .env files), Postgres backups.
