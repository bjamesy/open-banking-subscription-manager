...
| 6.8 | Data deletion is **implemented** (`DELETE /auth/me` — Plaid revocation best-effort, then a full local cascade delete in FK-safe order; no `ondelete` cascades at the DB level, done in application code). Consent logging is **implemented** (`GET`/`POST /consent`, `ConsentRecord` — append-only, versioned; gated before a fresh bank link in `ConnectBank.tsx`, disclosing the Anthropic data share). The disclosure text is placeholder copy, not lawyer-reviewed. | Data-residency review (Postgres location, Anthropic's US-based API) is still open — requires an actual privacy-law review, not just code, and depends on where this gets deployed. |
